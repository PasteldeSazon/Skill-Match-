...
// Validar formulário
function validarFormulario(nome, email, telefone, idade, area, habilidades, experiencia) {
  ...
  if (!experiencia || experiencia < 0) return "Experiência deve ser um número positivo.";
  return null;
}
...
async function renderizarVagas(candidato) {
  ...
  const vagas = motor.listaVagas;
  
  const vagasComScore = vagas.map((vaga) => ({
    vaga,
    score: motor.avaliarCandidato(candidato, vaga, false),
  }));
  
  // Ordenar por score (maior primeiro)
  vagasComScore.sort((a, b) => b.score - a.score);
  
  vagasComScore.forEach((item, index) => {
    const cor = item.score >= 80 ? "#4CAF50" : item.score >= 50 ? "#FFC107" : "#F44336";
    vagasContainer.innerHTML += `
...
form.addEventListener("submit", async (event) => {
  ...
  const experiencia = parseInt(document.getElementById("experiencia").value) || 0;
  
  // Validar
  const erro = validarFormulario(nome, email, telefone, idade, area, habilidades, experiencia);
  if (erro) {
    alert(erro);
    return;
  }
  ...
  // Criar candidato
  const candidato = new motor.Candidato(nome, idade, area, habilidades, experiencia);
  
  // Salvar no localStorage
  const perfil = { nome, email, telefone, idade, area, habilidades, experiencia };
